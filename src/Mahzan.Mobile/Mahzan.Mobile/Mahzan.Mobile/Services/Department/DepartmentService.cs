...
namespace Mahzan.Mobile.Services.Department
{
    public class DepartmentService: BaseService, IDepartmentService
    {
        ...
        public async Task<HttpResponseMessage> Create(CreateDepartmentCommand command)
        {
            HttpResponseMessage httpResponseMessage;
            UriBuilder uriBuilder = new UriBuilder(UrlApi + "/v1/Department/Create");
            try
            {
                HttpClient httpClient = new HttpClient();
        ...
        public async Task<HttpResponseMessage> Delete(string departmentId)
        {
            HttpResponseMessage httpResponseMessage;
            UriBuilder uriBuilder = new UriBuilder(UrlApi + "/v1/Department/Delete");
            try
            {
                var query = HttpUtility.ParseQueryString(uriBuilder.Query);
        ...
        public async Task<HttpResponseMessage> Update(UpdateDepartmentCommand command)
        {
            HttpResponseMessage httpResponseMessage;
            UriBuilder uriBuilder = new UriBuilder(UrlApi + "/v1/Department/Update");
            try
            {
                HttpClient httpClient = new HttpClient();
